...
public class FeedRepositoryQueryDSL {
  ...
  public List<Feed> getFeeds(FeedListRequest request) {
    return queryFactory
        ...
        .join(precipitation)
        .on(weather.precipitationId.eq(precipitation.id)) // weather, precipitation 조인
        .where(
  ...
  private BooleanExpression keywordLikeCondition(String keyword) {
    return keyword != null && !keyword.isBlank() ? feed.content.likeIgnoreCase("%" + keyword + "%")
        : null;
  }
  ...
  private BooleanExpression precipitationCondition(
